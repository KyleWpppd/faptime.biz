...
int main(void) {
    ...
    while (FCGI_Accept() >= 0) {
        ...
        if (is_valid_hash(request_hash) != 0) {
            not_found();
            break;
        }
...
int is_valid_hash(const char *hash) {
    ...
    // 0-length string
    if (strlen(hash) == 0) return HASH_EMPTY;
...
int faptime_encode(char * dest, const int url_id) {
    strcpy(dest, "");
    int remainder;
    int num = url_id;
    int i = 0;
    while (left > TABLE_SIZE-1) {
        remainder = num % TABLE_SIZE;
        num = num / TABLE_SIZE;
        dest[i] = remainder;
        if (++i > sizeof(dest)) return -1;
    }
    return i;
}
